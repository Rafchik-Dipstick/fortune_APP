...
The short mono PCM files under `assets/audio` are deterministic, low-amplitude interface feedback generated by `corepack npm run asset:audio`. `paper-draw.wav` accompanies accepted draw issuance and `reveal-shimmer.wav` accompanies a fresh card reveal; both are suppressed by the in-app Sound preference and never replace visible or accessible state. `corepack npm run asset:audio:validate` checks their container, duration, peak, and DC-offset bounds. The Expo audio plugin explicitly disables microphone and background-audio capabilities because Fortuneness uses playback only while active.
