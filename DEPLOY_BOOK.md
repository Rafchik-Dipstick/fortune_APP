...
- Canonicalized the intention-only body and hashed it with SHA-256. User/method/route/key records live for the account lifetime; exact successful and terminal retries parse and replay their bounded stored domain outcome, while changed input returns `IDEMPOTENCY_KEY_REUSED`. If an idempotency/draw uniqueness race still escapes the user lock, a new locked transaction loads and validates the committed record before replaying it.
...
- Added route tests and disposable PostgreSQL proof for sequential and 20-way simultaneous same-key replay, changed-input rejection, stable terminal replay, 51 simultaneous distinct-key requests, exactly one free plus ten paid-through subscription draws, exact subscription expiry, refunded-versus-later pack-grant isolation, and content rollback. All 15 integration tests and invariants passed, and the harness dropped its generated database.
...
- Fixed the Railway build command, which compiled `@fortuneness/api-contracts` and `@fortuneness/api` but not `@fortuneness/fortune-content`. Every workspace package resolves to its own `dist/`, and `dist/` is gitignored, so on Railway's fresh clone nothing is built until the build command names it. The running server does not import `fortune-content` — its import graph from `dist/main.js` reaches only `api-contracts`, so the service would have booted and passed its health check — but `prisma/seed.ts` does import it, so the content seed would have failed with an unresolved module and left the database with no cards to draw. Also corrected `watchPatterns`, which watched `packages/shared-types/**`, a package the API does not use, while ignoring `packages/fortune-content/**`, a package it depends on: a content-only change could not have triggered a redeploy.
