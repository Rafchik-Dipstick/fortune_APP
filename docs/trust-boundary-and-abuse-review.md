...
Scope: Sign in with Apple identity, app sessions, account switching, fortune allowance, StoreKit delivery, App Store notifications, credit/refund accounting, offline caches, and account deletion
...
Phase 5 identity work requires a physical-device Sign in with Apple spike and verified capability/provisioning. Phase 9 commerce work cannot be accepted until the Xcode/Sandbox trust separation, product configuration, verified JWS spike, and App Store notification setup are proven. A failed proof, changed Apple platform requirement, or control that cannot be implemented reopens this review and blocks the owning phase.
...
1. One verified Sign in with Apple subject maps to one current Fortuneness user; unsigned, wrong-audience, wrong-nonce, stale, or replayed tokens never create or select an account.
...
| Raw Apple identity token                          | Ephemeral identity evidence          | TLS in transit, bounded in memory, JWT/nonce/freshness/replay validation, discard after verification, never log.                               |
| Sign in with Apple subject                        | Pseudonymous identity                | Domain-separated versioned HMAC digests only after verification.                                                                               |
...
AuthenticationServices and StoreKit results cross from Apple frameworks into Expo/native modules and then TypeScript. Only server-verified Apple identity tokens and framework-verified StoreKit results are trusted. Refresh tokens and the active purchase token belong in Keychain/account-scoped memory; reading cache belongs in account-partitioned SQLite with iOS Data Protection. A deliberate disconnect clears the previous account's memory and SQLite partition before another identity can become active.
...
### TB-C — Sign in with Apple token and JWKS retrieval
...
| ----- | ---------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | -------------------------------- |
| AC-01 | Attacker attempts to redirect identity-key retrieval or substitute a signing key.                                                  | Critical | The Apple JWKS URL is a fixed code constant; HTTPS, `kid`, `RS256`, issuer, audience, timeout, and signature are enforced.                                                                                                                                    | Phase 5 verifier tests and egress inventory.                                               | Design-mitigated                 |
| AC-02 | Stale, replayed, malformed, wrong-audience, or wrong-nonce Apple token creates or takes over an account.                           | Critical | Configured freshness/skew; bounded replay fingerprint; exact bundle audience; exact nonce; Apple JWKS signature validation; atomic identity upsert.                                                                                                           | Physical-device spike; Phase 5 negative/concurrency tests.                                 | External gate open               |
...
| AC-05 | A different Apple Account is selected while requests/cache from the previous account remain active.                                | Critical | Sensitive reauthentication checks the local identity fingerprint; deliberate disconnect clears memory, Keychain-scoped purchase token, and SQLite partition before a new bootstrap.                                                                           | Phase 5 mismatch tests; Phase 7 cache-isolation tests; Phase 9 ownership tests.            | Design-mitigated / evidence open |
...
| AC-13 | Xcode test certificate, Sandbox event, or Production event is accepted in another environment.                                     | Critical | Explicit receiver environment; environment in every business key; local-only Xcode certificate/config; staging Sandbox only; production accepts Production plus cryptographically distinct Sandbox for App Review/TestFlight; startup fail-closed validation. | Phase 0 Xcode/Sandbox spike; Phase 9 trust-crossover tests; deployment secret audit.       | External gate open               |
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Phase 0     | Physical iPhone Sign in with Apple token; signed IPA Apple-sign-in entitlement; Xcode-versus-Sandbox JWS distinction; `appAccountToken` receipt; final IDs/configuration record.                                                                 |
...
| Phase 5     | Apple JWT/nonce/replay/rotation/concurrency tests; rotating refresh-family tests; immediate authoritative invalidation; account switching.                                                                                                       |
...
- launch identity provider, external-identity linking, or Apple subject transfer choice;
